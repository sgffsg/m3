...
{
    public class VerticalCombine : CombinePattern
    {
        public override bool CheckPattern(Chip bonusChip, Chip otherChip = null)
        {
            markedChips.Clear();
            if (bonusChip.ChipBonusType != ChipBonusType.Vertical)
                return false;

            markedChips = GameField.Instance.GetColumn(bonusChip.Position.y);
